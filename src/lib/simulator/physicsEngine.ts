...
export function updateMotionState(
  ...
  const rudderAngleRad =
    ((controls.portRudder + controls.stbdRudder) / 2) * (Math.PI / 180);
  ...
  const rudderForceY =
    ...
    flowSpeed *
    rudderLiftCoeff *
    Math.sin(rudderAngleRad);
  const rudderMomentN = rudderForceY * shipData.rudder.location.x;

  // Propeller thrust
  const thrustCoeff = 25000; // N per percent
  const thrustForceX =
    ((controls.portEngine + controls.stbdEngine) / 2) * thrustCoeff;

  // Hydrodynamic drag (simplified quadratic model)
  const dragCoeffX =
    shipData.principalParticulars.blockCoefficient *
    1.025 *
    shipData.principalParticulars.breadth *
    shipData.principalParticulars.draft *
  ...
  const addedMassY = shipData.massAndInertia.mass * 0.2;
  ...
  const sog = Math.sqrt(
    velOverGround_North * velOverGround_North +
...
export function interpolateMotion(
  ...
  const headingDiff = angularDifference(
    prev.position.heading,
    current.position.heading
  );
  const interpolatedHeading = normalizeAngle(
    prev.position.heading + headingDiff * alpha
  );
